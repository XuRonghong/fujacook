        ...
        try{
            if ($from=='order') {
                ...
                $orderDetail = OrderDetail::query()->create([
                    'no' => $order->no,
                    'order_id' => $order->id,
                    'product_id' => isset($attributes['product_id']) ?: '1',
                    'ownerKey' => isset($attributes['ownerKey']) ?: '1',
                    'related' => isset($attributes['related']) ?: 'product_specs',
                    'type' => 0,
                    'purchase_price' => 20,
                    'cost_price' => 10,
                    'price' => 15,
                ...
                $orderContact = OrderContact::query()->create([
                    ...
                    'no' => $order->no,
                    'type' => 1,
                    'name' => data_get($attributes,'name', auth()->guard('admin')->user()->name),
                    'gender' => 'man',
                    'email' => data_get($attributes,'email', auth()->guard('admin')->user()->email),
                    'phone' => data_get($attributes,'phone', '090000000'),
                ]);
                $orderContact = OrderContact::query()->create([
                    'order_id' => $order->id,
                    'no' => $order->no,
                    ...
                    'gender' => 'man',
                    'email' => data_get($attributes,'email', auth()->guard('admin')->user()->email),
                    'phone' => data_get($attributes,'phone', '090000001'),
                ]);
                return 'ok';
            } else {
