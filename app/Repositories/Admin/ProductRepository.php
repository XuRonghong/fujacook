...
class ProductRepository extends Repository
{
    ...
    {
        return Product::query()->whereRaw($whereQuery)->orderBy('rank', 'asc')->get($columns);
    }
    ...
    public function delete($id, $onDelete='', $for='')
    {
        try{
             ...
             */
            if ($onDelete=='cascade' && $for=='product_spec') {
                $this->setModel_ProductSpec();
                $model = $this->model->where('product_id', $id)->delete();

                $author_id = auth()->guard('admin')->user()->id;
                $value = json_encode( $model, JSON_UNESCAPED_UNICODE);
                FuncController::addActionLog('delete', $author_id, $value,'product_id='.$id, $this->model->getTable() );
            }
